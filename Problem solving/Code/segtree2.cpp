...
int query(int node, int l, int h, int qL, int qR) {
    ...
    int p2 = query(Right, mid + 1, h, qL, qR);
    { // operation
      return p1 + p2; 
    }
}
void update(int node, int l, int h, int pos, int newvalue) {
    if (pos > h || pos < l)
        return; 
    if (l >= pos && h <= pos) {
        tree[node] = newvalue;
    ...
    update(Right, mid + 1, h, pos, newvalue);
    { // operation
      tree[node] = tree[Left] + tree[Right];
    }
}
